...
/**
 * Loads the Applications list. The section reads one domain, so the load belongs to the slice; a
 * screen spanning several would own it instead — see `.claude/rules/stores.md`.
 *
 * ! Refresh, a server event and a reconnect all retrigger it, so the previous load is cancelled and
 * ! its answer dropped: without this the slower of two requests decides what the list shows.
 */
...
/**
 * The first visit's load, and nothing on a later one: the list is the whole set and a section that
 * comes back to it reads what it already has. Refresh and the service call `loadApplications`.
 */
...
/**
 * Refetches one application after a lifecycle command or a server event, leaving the rest of the
