/**
 * The admin events hub, browser side — the shell's own subscriptions only; a section imports the
 * platform's client itself. That client (`<admin:events>/client.js`) rides a shared worker keyed
 * by the script url, so every consumer on the page shares one socket. Per-topic authorization is
 * the server's (`setTopic` `allow`), never re-decided here.
 */
