...
beforeEach(() => {
  ...
  vi.mocked(fetchRolesScreen).mockReturnValue(
    answered({ roles: [ROLE], idProviders: [PROVIDER], projects: [PROJECT] }),
  );
});
...
describe('loadRolesScreen', () => {
  ...
  it('fails only the domain whose field came back null', async () => {
    vi.mocked(fetchRolesScreen).mockReturnValue(
      answered({ roles: [ROLE], idProviders: [PROVIDER], projects: null }, 'Project repo is down'),
    );
  ...
  it('drops the answer of the load a newer one replaced', async () => {
    ...
    vi.mocked(fetchRolesScreen)
      ...
      .mockReturnValueOnce(answered({ roles: [], idProviders: [], projects: [PROJECT] }));

    const slow = loadRolesScreen();
    await loadRolesScreen();
    answerSlowly?.({ data: { roles: [ROLE], idProviders: [PROVIDER], projects: [] } });
    await slow;
