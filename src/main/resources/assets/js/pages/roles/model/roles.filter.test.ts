...
const projects: Project[] = [{ id: 'intranet', displayName: 'Company intranet' }];
...
describe('roleBuckets', () => {
  ...
  it('offers system first, custom second, then the projects', () => {
    expect(roleBuckets(all, all, projects, labels).map(({ id }) => id)).toEqual([
      SYSTEM_BUCKET,
      CUSTOM_BUCKET,
  ...
  it('labels the fixed buckets from the phrases and a project from its display name', () => {
    expect(roleBuckets(all, all, projects, labels).map(({ label }) => label)).toEqual([
      'System',
      'Custom',
      'Company intranet',
    ]);
  ...
  it('counts the roles of each bucket', () => {
    expect(roleBuckets(all, all, projects, labels).map(({ count }) => count)).toEqual([2, 1, 2]);
  });

  it('counts the matched roles, so the counts follow the query', () => {
    const searched = searchRoles(all, 'intranet');

    expect(roleBuckets(all, searched, projects, labels).map(({ count }) => count)).toEqual([
      0, 0, 2,
    ]);
  });

  // ! A ticked project bucket has to survive a projects list that failed to load, or the list stays
  // ! narrowed by an entry that is gone from the menu, with no way left to untick it.
  it('offers a project bucket from the roles alone when the project list is empty', () => {
    ...
    // Labelled by the id, since no display name is available to read.
  ...
  it('offers a project with no roles of its own, at zero', () => {
    const blank: Project[] = [{ id: 'blank', displayName: 'Blank' }];

    const offered = roleBuckets(all, all, blank, labels);

    expect(offered.find(({ id }) => id === projectBucketId('blank'))).toEqual({
      id: projectBucketId('blank'),
      label: 'Blank',
      count: 0,
    });
    // And the project the roles name is still there beside it, unknown to that list.
    expect(offered.map(({ id }) => id)).toContain(projectBucketId('intranet'));
  });
});
